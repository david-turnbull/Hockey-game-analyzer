...
def run_diagnostics():
    ...
    with app.app_context():
        ...
        event_ids = {ev[0] for ev in db.session.query(Event.event_id).all()}
        orphan_shots_count = db.session.query(Shot).filter(~Shot.shot_id.in_(list(event_ids))).count() if event_ids else Shot.query.count()
        ...
        integrity_issues_sum = (orphan_events_count + orphan_shots_count + orphan_shifts_count + 
                                invalid_player_refs + negative_duration_shifts + shots_without_shooters)
        ...
        print(f"Orphan shots: {orphan_shots_count:>11}")
        print(f"Orphan shifts: {orphan_shifts_count:>10}")
        ...
        print(f"Unknown situations: {invalid_manpower_states:>7}")  # maps to invalid manpower states
